...
test.describe("Invite Cascade Deletion Behavior", () => {
	test("used invite accepted, others marked joined_elsewhere", async ({
		...
		await expect(
			pageB.getByText("Joined via another invite"),
		).toBeVisible();
	...
	test("inviter deletes account - invite still works", async ({
		browser,
	}) => {
	...
	test("invite count accuracy with joined_elsewhere", async ({
		browser,
	}) => {
		const inviterA = await createUser({
	...
	test("revoked invite no longer works", async ({ browser }) => {
		...
		await expect(
			inviteePage.getByText(
				/this invite link is invalid or doesn't exist/i,
			),
		).toBeVisible();
	...
	test("UI displays all status types correctly", async ({ browser }) => {
		...
		await expect(
			pageA.getByText("Joined via another invite"),
		).toBeVisible();
