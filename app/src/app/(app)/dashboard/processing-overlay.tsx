 ...
 * an "Analyzing" spinner pill while `processing`, a "Queued" pill while
 * `deferred` (parked for budget — resumes on the daily sweep), a red "Failed"
 * badge on `failed`. Fades out once `completed` so the card settles rather than
 * popping. The scrim is pointer-transparent so the card underneath stays
 * clickable (only the pill captures pointer events, to keep its own cursor).
 */
export function ProcessingOverlay({ status }: { status: ProcessingStatus }) {
  ...
  return (
    <AnimatePresence>
      {(isProcessing || isDeferred || isFailed) && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        ...
        >
          ...
          <div
            className={cn(
              ...
              isDeferred && "bg-amber-500/80 text-white",
